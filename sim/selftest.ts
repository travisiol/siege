...
console.log("\n1. Integer square root (Babylonian)");
...
console.log("\n2. Cohesion bounds (100..150)");
ok(cohesion(0) === 100n, "0 active members -> 100");
ok(cohesion(25) === 150n, "25 members -> 150");
ok(cohesion(999) === 150n, "caps at 150");

console.log("\n3. Fortification bounds (100..200)");
ok(fortification(10, 10) === 100n, "freshly taken hex -> 100");
ok(fortification(30, 10) === 200n, "held 20 ticks -> 200");
ok(fortification(200, 10) === 200n, "caps at 200");

console.log("\n4. Empire tax -- the brief's own three examples");
...
ok(A === D, "equal power", `${fmt(A)} = ${fmt(D)}`);
ok(!attackerWins(A, D), "tie -> the defender holds");

console.log("\n7. Rebellion buff (<3 hexes and tick > 32)");
...
ok((noBuff * 125n) / 100n === buff, "+25% from tick 33 on", `${fmt(noBuff)} -> ${fmt(buff)}`);

console.log("\n8. Map");
const map = buildMap(13);
ok(map.cells.length === 547, "radius 13 -> 547 hexes (the brief claims 512)", String(map.cells.length));
...
ok(adjacentT3 === 0, "no tier 3 adjacent to another", `${adjacentT3} pairs`);
const isolated = map.cells.filter((c) => c.neighbors.length < 3).length;
ok(isolated === 0, "no isolated hex (connectivity)", `${isolated}`);
